...
int32 set_uniform_value(lua_State* lua)
{
    ...
    auto value = (real32)lua_tonumber(lua, 3);
    auto command = render_interface::create_command(RendererCommand::SetUniformValue);
    ...
    command.data = suvd;
    render_interface::dispatch(s_render_interface, &command, &value, sizeof(real32));
    return 0;
}
