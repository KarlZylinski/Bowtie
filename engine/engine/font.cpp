...
namespace font
{
	...
	Rect char_uv(const Font* font, char c)
	{
		...
		auto char_uv_min = vector2::create(char_pos.x / (float)font->texture->image->resolution.x, char_pos.y / (float)font->texture->image->resolution.y);
		auto char_uv_max = vector2::add(&char_uv_min, &vector2::create(char_size.x / (float)font->texture->image->resolution.x, char_size.y / (float)font->texture->image->resolution.y));
		Rect rect;
